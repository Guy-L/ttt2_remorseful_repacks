...
local function DeWorm(ply)
	ply.infected = false
	ply:SetNWBool('infected',false)

	timer.Remove(ply:EntIndex().."BigDeath")
	timer.Remove(ply:EntIndex().."Poison")
	-- just in case strange timing:
	if IsValid(ply) then ply:ConCommand("-attack") end

	local promise_name = "BrainParasite_DeWorm"..ply:EntIndex()
	hook.Remove("TTTEndRound", promise_name)
	hook.Remove("TTTPrepareRound", promise_name)
	hook.Remove("TTTBeginRound", promise_name)
end
...
function SWEP:PrimaryAttack(worldsnd)
	...
	bullet.Callback = function(att, tr, dmginfo)
		...
		if SERVER and IsValid(ply) and (ply:IsPlayer() or ply:IsNPC()) then
			...
			-- else
			timer.Create(ply:EntIndex().."BigDeath",20,1,function() DoPoison(attacker,ply) end)
			timer.Create(ply:EntIndex().."Poison",0.5,20*2,function()
				if IsValid(ply) and ply.infected and ply.Alive and ply:Alive() then
					ply:ConCommand("+attack")
					timer.Simple(0.25,function() if IsValid(ply) then ply:ConCommand("-attack") end end)
				end
			end)
			local promise = function() DeWorm(ply) end
			local promise_name = "BrainParasite_DeWorm"..ply:EntIndex()
			hook.Add("TTTEndRound", promise_name, promise)
			hook.Add("TTTPrepareRound", promise_name, promise)
			hook.Add("TTTBeginRound", promise_name, promise)
